...
@interface MMMTDealloc : NSObject

@property (nonatomic) MMMTRule *rule;
@property (nonatomic) Class cls;
@property (nonatomic) pthread_mutex_t invokeLock;

@end

@implementation MMMTDealloc

- (void)dealloc
{
    SEL selector = NSSelectorFromString(@"discardRule:whenTargetDealloc:");
    ((void (*)(id, SEL, MMMTRule *, MMMTDealloc *))[MMMTEngine.defaultEngine methodForSelector:selector])(MMMTEngine.defaultEngine, selector, self.rule, self);
}
...
- (BOOL)apply
{
    return [MMMTEngine.defaultEngine applyRule:self];
}

- (BOOL)discard
{
    return [MMMTEngine.defaultEngine discardRule:self];
}

- (MMMTDealloc *)mt_deallocObject
{
    MMMTDealloc *mtDealloc = objc_getAssociatedObject(self.target, self.selector);
    return mtDealloc;
}

@end

@interface MMMTEngine ()

@property (nonatomic) NSMapTable<id, NSMutableSet<NSString *> *> *targetSELs;
@property (nonatomic) NSMapTable *aliasSelectorCache;

- (void)discardRule:(MMMTRule *)rule whenTargetDealloc:(MMMTDealloc *)mtDealloc;

@end

@implementation MMMTEngine
...
+ (instancetype)defaultEngine
{
    ...
    static MMMTEngine *instance;
    dispatch_once(&onceToken, ^{
        instance = [MMMTEngine new];
    });
    return instance;
}
...
- (NSArray<MMMTRule *> *)allRules
{
    ...
    for (id target in [[self.targetSELs keyEnumerator] allObjects]) {
        ...
        for (NSString *selectorName in selectors) {
            MMMTDealloc *mtDealloc = objc_getAssociatedObject(target, NSSelectorFromString(selectorName));
...
- (BOOL)discardRule:(MMMTRule *)rule
{
    pthread_mutex_lock(&mutex);
...
static BOOL mt_checkRuleValid(MMMTRule *rule)
{
    if (rule.target && rule.selector && rule.durationThreshold > 0) {
        ...
        if ([className isEqualToString:@"MMMTRule"] || [className isEqualToString:@"MMMTEngine"]) {
            return NO;
        }
        return YES;
    }
...
static SEL mt_aliasForSelector(SEL selector)
{
    ...
    SEL aliasSelector = (__bridge void *)[MMMTEngine.defaultEngine.aliasSelectorCache objectForKey:(__bridge id)(void *)selector];
    if (!aliasSelector) {
        NSString *selectorName = NSStringFromSelector(selector);
        aliasSelector = NSSelectorFromString([NSString stringWithFormat:@"__mt_%@", selectorName]);
        [MMMTEngine.defaultEngine.aliasSelectorCache setObject:(__bridge id)(void *)aliasSelector forKey:(__bridge id)(void *)selector];
    }
    pthread_mutex_unlock(&alias_selector_mutex);
    return aliasSelector;
}

static BOOL mt_invokeFilterBlock(MMMTRule *rule, NSInvocation *originalInvocation)
{
...
{
    MMMTDealloc *mtDealloc = objc_getAssociatedObject(invocation.target, invocation.selector);
    MMMTRule *rule = mtDealloc.rule;
    if (!rule) {
...
static void mt_forwardInvocation(__unsafe_unretained id assignSlf, SEL selector, NSInvocation *invocation)
{
    ...
    MMMTDealloc *mtDealloc = objc_getAssociatedObject(invocation.target, originalSelector);
...
static BOOL mt_recoverMethod(id target, SEL selector)
{
    ...
    if (mt_object_isClass(target)) {
        cls = target;
        if ([MMMTEngine.defaultEngine containsSelector:selector onTargetsOfClass:cls]) {
            return NO;
        }
    }
    else {
        cls = object_getClass(target);
        if ([MMMTEngine.defaultEngine containsSelector:selector onTarget:cls] ||
            [MMMTEngine.defaultEngine containsSelector:selector onTargetsOfClass:cls]) {
            return NO;
        }
    }
...
static void mt_configureTargetDealloc(MMMTRule *rule)
{
    Class cls = object_getClass(rule.target);
    MMMTDealloc *mtDealloc = [rule mt_deallocObject];
    if (!mtDealloc) {
        mtDealloc = [MMMTDealloc new];
        mtDealloc.rule = rule;
        mtDealloc.cls = cls;
...
- (NSArray<MMMTRule *> *)mt_allRules
{
    NSMutableArray<MMMTRule *> *result = [NSMutableArray array];
    for (MMMTRule *rule in MMMTEngine.defaultEngine.allRules) {
...
- (nullable MMMTRule *)mt_limitSelector:(SEL)selector oncePerDuration:(NSTimeInterval)durationThreshold
{
    return [self mt_limitSelector:selector oncePerDuration:durationThreshold usingMode:MMMTPerformModeDebounce];
}

- (nullable MMMTRule *)mt_limitSelector:(SEL)selector oncePerDuration:(NSTimeInterval)durationThreshold usingMode:(MMMTPerformMode)mode
{
...
- (nullable MMMTRule *)mt_limitSelector:(SEL)selector oncePerDuration:(NSTimeInterval)durationThreshold usingMode:(MMMTPerformMode)mode onMessageQueue:(dispatch_queue_t)messageQueue
{
...
- (nullable MMMTRule *)mt_limitSelector:(SEL)selector oncePerDuration:(NSTimeInterval)durationThreshold usingMode:(MMMTPerformMode)mode onMessageQueue:(dispatch_queue_t)messageQueue alwaysInvokeBlock:(id)alwaysInvokeBlock
{
    MMMTDealloc *mtDealloc = objc_getAssociatedObject(self, selector);
    MMMTRule *rule = mtDealloc.rule;
    if (!rule) {
        rule = [[MMMTRule alloc] initWithTarget:self selector:selector durationThreshold:durationThreshold];
    }
